...
class StormLcmSpec extends HealthCheckSpecification {
    ...
    def "System's able to fail an ISL if switches on both ends go offline during restart of network topology"() {
        ...
        def srcBlockData = lockKeeper.knockoutSwitch(islUnderTest.srcSwitch, mgmtFlFactory)
        def dstBlockData = lockKeeper.knockoutSwitch(islUnderTest.dstSwitch, mgmtFlFactory)
