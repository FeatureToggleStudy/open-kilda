...
public class ValidateFlowAction extends NbTrackableAction<FlowUpdateFsm, State, Event, FlowUpdateContext> {
    ...
    protected Optional<Message> performWithResponse(State from, State to, Event event, FlowUpdateContext context,
        ...
        if (!isOperationAllowed) {
            throw new FlowProcessingException(ErrorType.NOT_ALLOWED, getGenericErrorMessage(),
        ...
        try {
            ...
            throw new FlowProcessingException(e.getType(), "Flow validation error", e.getMessage());
        } catch (UnavailableFlowEndpointException e) {
            throw new FlowProcessingException(ErrorType.DATA_INVALID, "Flow validation error", e.getMessage());
        }

        if (targetFlow.getDiverseFlowId() != null
                && targetFlow.getSrcSwitch().equals(targetFlow.getDestSwitch())) {
            throw new FlowProcessingException(ErrorType.DATA_INVALID, "Flow validation error",
        ...
        saveHistoryWithEvent(stateMachine, "Flow was validated successfully", FlowEventData.Event.UPDATE);

        return Optional.empty();
    }

    protected void onFailure(FlowUpdateFsm stateMachine, String errorMessage, Exception ex) {
        dashboardLogger.onFailedFlowUpdate(stateMachine.getFlowId(), errorMessage);
    }
