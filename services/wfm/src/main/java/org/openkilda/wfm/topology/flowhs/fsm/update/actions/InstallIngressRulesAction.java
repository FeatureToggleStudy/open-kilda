...
public class InstallIngressRulesAction extends FlowProcessingAction<FlowUpdateFsm, State, Event, FlowUpdateContext> {
    ...
    protected void perform(State from, State to, Event event, FlowUpdateContext context, FlowUpdateFsm stateMachine) {
        ...
        if (commands.isEmpty()) {
            log.debug("No install commands for ingress rules for the flow {}", flowId);

            saveHistory(stateMachine, "No install commands for ingress rules.");

            stateMachine.fire(Event.INGRESS_IS_SKIPPED);
        } else {
            log.debug("Commands for installing ingress rules have been sent for the flow {}", flowId);
