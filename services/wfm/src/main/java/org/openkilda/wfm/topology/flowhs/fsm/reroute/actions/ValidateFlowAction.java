...
public class ValidateFlowAction extends NbTrackableAction<FlowRerouteFsm, State, Event, FlowRerouteContext> {
    ...
    protected Optional<Message> performWithResponse(State from, State to, Event event, FlowRerouteContext context,
        ...
        saveHistoryWithEvent(stateMachine, "Flow was validated successfully", FlowEventData.Event.REROUTE);

        return Optional.empty();
    }

    protected void onFailure(FlowUpdateFsm stateMachine, String errorMessage, Exception ex) {
        dashboardLogger.onFailedFlowReroute(stateMachine.getFlowId(), errorMessage);
    }
